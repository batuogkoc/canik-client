...
Future<List<Map<String, dynamic>>> csvRead(String path) async {
  ...
  var list = await File(path)
    ...
    var quatVal = Quaternion(list[6], list[7], list[8], list[9]);
    double dt = list[10];
...
Future<List<Map<String, dynamic>>> csvReadOld(String path) async {
  var madgwickTransformer = RawDataToProcessedDataTransformer(
      ScfAhrs({"aLambda1": 0.1, "aLambda2": 1}));
  var scfTransformer =
      RawDataToProcessedDataTransformer(MadgwickAhrs({"beta": 0.5}));
