...
class FscfCcAhrs implements Ahrs {
  ...
  void updateIMU(Vector3 gyroRad, Vector3 accel, double dt) {
    ...
    qDot = qDot.scaled(0.5);
    // Quaternion qDot = Quaternion.dq(_quaternion, gyroRad);

    Quaternion qp = _quaternion + qDot.scaled(dt);
    // qp.normalize();
    ...
    Vector3 accMesPred = Vector3(2 * (qp.x * qp.z - qp.w * qp.y),
        2 * (qp.w * qp.x + qp.y * qp.z), 2 * (0.5 - qp.x * qp.x - qp.y * qp.y));

    final veca = accel.cross(accMesPred).normalized();
    final aPlusm = veca * (aLambda1 / 2);
    final qCor = Quaternion(aPlusm.x, aPlusm.y, aPlusm.z, 1);
